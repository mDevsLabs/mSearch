...
async function afterPackageBuilt (packagePath) {
  ...
  /* create zip files */
  var output = fs.createWriteStream('dist/app/' + 'mSearch-v' + version + '-windows' + archSuffix + '.zip')
  var archive = archiver('zip', {
    zlib: { level: 9 }
  })
  archive.directory(packagePath, 'mSearch-v' + version)
  archive.pipe(output)
  ...
  await installer(options)
    .then(function () {
      fs.renameSync('./dist/app/msearch-installer' + archSuffix + '/msearch-' + version + '-setup.exe', './dist/app/msearch-' + version + archSuffix + '-setup.exe')
    })
    .catch(err => {
