...
require('./createPackage.js')('mac', { arch: toArch(platform) }).then(function (packagePath) {
  if (platform === 'arm64') {
    execSync('codesign -s - -a arm64 -f --deep ' + packagePath + '/mSearch.app')
  }
  ...
  var output = fs.createWriteStream('dist/app/mSearch-v' + version + '-mac-' + platform + '.zip')
  var archive = archiver('zip', {
    zlib: { level: 9 }
  })

  archive.directory(path.resolve(packagePath, 'mSearch.app'), 'mSearch.app')
